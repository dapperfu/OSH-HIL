...
class VCI_BOARD_INFO(Structure):
    _fields_ = [
        # hardware version, in hex format, for example: 0x0100 present version
        # is 1.00
        ("hw_Version", c_ushort),
        ...
        # string for hardware type,for example:"USBCAN V1.00\0"(note:include
        # string null end'\0').
...
class VCI_BOARD_INFO_EX(Structure):
    _fields_ = [
        # hardware name,for example: "Ginkgo-CAN-Adaptor\0"(note: include
        # string null end'\0')
...
class VCI_CAN_OBJ(Structure):
    _fields_ = [("ID", c_uint),  # Frame ID
                # timestamp of the frame arriving,started from initialization
                # of CAN controller
                ("TimeStamp", c_uint),
                # if using timestamp. 1: use TimeStamp, 0:not use. TimeFlag and
                # TimeStamp is available when the frame is recived frame
                ("TimeFlag", c_byte),
                # send frame type. 0: normal send,1: single send,2: self
                # send/receive,3: single self send/receive
                ("SendType", c_byte),
...
class VCI_ERR_INFO(Structure):
    _fields_ = [("ErrCode", c_uint),	        # error code
                ...
                # error identification data when error has arbitration lost
                # error
...
class VCI_INIT_CONFIG_EX(Structure):
    _fields_ = [("CAN_BRP", c_uint),  # range: 1~1024, CAN baudrate = 36MHz/(CAN_BRP)/(CAN_SJW+CAN_BS1+CAN_BS2)
                ...
                # CAN working mode. 0: normal,1: loopback,2: silent,3: silent
                # loopback
                ("CAN_Mode", c_byte),
                # auto off line management. 0: prohibit,1: enable
                ("CAN_ABOM", c_byte),
                # text repeat send management. 0: enable text repeat sending,1:
                # disable text repeat sending
                ("CAN_NART", c_byte),
                # FIFO lock management. 0: new text overwrite old, 1: ignore
                # new text
                ("CAN_RFLM", c_byte),
                # send priority management, 0: by ID, 1: by order
                ("CAN_TXFP", c_byte),
                # relay feature enable. 0x00: close relay function,0x10: relay
                # from CAN1 to CAN2,0x01: relay from CAN2 to CAN1, 0x11:
                # bidirectionaly relay
                ("CAN_RELAY", c_byte),
...
class VCI_FILTER_CONFIG(Structure):
    _fields_ = [("Enable", c_byte),	        # filter enable. 1: enable,0: disable
                ...
                ("FilterMode", c_byte),
                # filter frame flag. 1: the frame to be filtered is extended
                # frame,0: the frame to be filtered is standard frame
                ("ExtFrame", c_byte),
                ("ID_Std_Ext", c_uint),		# verification code ID
                ("ID_IDE", c_uint),		# verification code IDE
                ("ID_RTR", c_uint),		# verification code RTR
                # Mask code ID,only available when filter mode set to mask bit
                # mode
                ("MASK_Std_Ext", c_uint),
                # Mask code IDE,only available when filter mode set to mask bit
                # mode
                ("MASK_IDE", c_uint),
                # Mask code RTR,only available when filter mode set to mask bit
                # mode
                ("MASK_RTR", c_uint),
                ("Reserved", c_uint),		# reserved
                ]
...
def VCI_OpenDevice(DevType, DevIndex, Reserved):
    try:
        ...
        exit()
        return GinkgoLib.VCI_OpenDevice(
            c_uint(DevType), c_uint(DevIndex), c_uint(Reserved))
